...
const IconBox: FC<IconBoxProps> = ({
    ...
    let iconBoxMedia
    if (media) {
        iconBoxMedia = media === 'image' ?
            <div className={iconBoxMediaClass} style={{backgroundImage: 'url(' + imageSrc + ')'}}></div> :
            <div className={iconBoxMediaClass}><i className={media}></i></div>
    }

    if (button) {
        return (
            <button className={iconBoxClass} onClick={onClick} type={buttonType ?? 'button'} {...props}>
                {iconBoxMedia}
